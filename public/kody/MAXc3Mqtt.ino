...
const char* mqtt_topic_cmd = "esp32/tekstowy/gniezno2024/cmd";      
const char* mqtt_topic_status = "esp32/tekstowy/gniezno2024/status"; 
// Client ID z MAC adresem dla unikalności
...
// NOWA ZMIENNA - kontrola włączenia/wyłączenia
...
void mqttCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (message == "OFF") {
    ...
    Serial.println("✓ Wyswietlacz WYLACZONY (hardware off)");
  }
  ...
  // WŁĄCZ WYŚWIETLACZ
    ...
    Serial.println("✓ Wyswietlacz WLACZONY");
  }
  ...
  // TEKST: + treść
    ...
    mqttClient.publish(mqtt_topic_status, ("TEXT: " + text).c_str());
  ...
  // JASNOŚĆ: BRIGHTNESS:10
    ...
    if (brightness >= 0 && brightness <= 15) {
      ...
      mqttClient.publish(mqtt_topic_status, ("BRIGHTNESS:" + String(brightness)).c_str());
  ...
  // PRĘDKOŚĆ: SPEED:50
    ...
    if (speed >= 10 && speed <= 150) {
      scrollSpeed = speed;
      myDisplay.setSpeed(scrollSpeed);
      mqttClient.publish(mqtt_topic_status, ("SPEED:" + String(speed)).c_str());
...
<body>
    <div class="container">
        <div class="header">
            ...
            <small>ESP32-C3 SuperMini + MQTT</small>
        </div>
    ...
    <script>
        ...
        function sendText() {
            ...
            }).then(() => showStatus('✓ Wysłano!', true));
        }

        function stopScroll() {
            fetch('/stop').then(() => showStatus('⏸ Stop', true));
        }
        ...
        function showDateNow() {
            fetch('/showdate').then(() => showStatus('📅 Data', true));
        }

        function showWeatherNow() {
            fetch('/showweather').then(() => showStatus('🌤️ Pogoda', true));
        }
...
void setup() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    mqttClient.setSocketTimeout(10); // Timeout 10s
    mqttReconnect();
  } else {
...
void loop() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    if (!mqttClient.connected()) {
      ...
      if (now - lastReconnectAttempt > 15000) { // Próba co 15s (było 10s)
